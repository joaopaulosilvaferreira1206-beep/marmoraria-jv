...
export default function Fornecedores() {
    ...
    async function excluir(id) {
        const confirmado = await popup.confirm('Deseja excluir este fornecedor?')
        ...
        if (error) {
            if (error.code === '23503') {
                await popup.showWarning(
                    'Este fornecedor não pode ser excluído pois está vinculado a entradas ou pedidos.\n\nRemova os vínculos antes de excluí-lo.'
                )
            } else {
                await popup.showError(`Erro ao excluir fornecedor: ${error.message}`)
            }
            return
        }
    ...
    function abrirEditar(f) {
        setForm({ nome: f.nome, telefone: f.telefone || '', email: f.email || '', endereco: f.endereco || '' })
        setEditando(f.id)
        setModal(true)
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-end">
                <button
                    onClick={() => { setForm(fornecedorVazio); setEditando(null); setModal(true) }}
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex gap-2 justify-center">
                                        ...
                                        {pode.apagarRegistros && (
                                            <button onClick={() => excluir(f.id)} className="text-red-400 hover:text-red-300">
                                                <Trash2 size={16} />
                                            </button>
                                        )}
                                    </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-xl p-6 w-full max-w-md">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setModal(false)} className="text-gray-400 hover:text-gray-200">
                                <X size={20} />
                            </button>
                        </div>
