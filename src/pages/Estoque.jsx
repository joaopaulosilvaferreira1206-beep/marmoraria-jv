...
export default function Estoque() {
    ...
    async function carregarMateriais() {
        setLoading(true)
        const { data } = await supabase.from('materiais').select('*').order('descricao')
        setMateriais(data || [])
        setLoading(false)
    }
    ...
    async function excluir(id) {
        const confirmado = await popup.confirm('Deseja excluir este material? Todos os registros relacionados também serão removidos.')
        ...
        if (error) { popup.showError('Erro ao excluir material: ' + error.message); return }

        popup.showSuccess('Material excluído com sucesso!')
        carregarMateriais()
    }
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-2 justify-end">
                <button onClick={() => exportarEstoquePDF(materiais)}
                    className="flex items-center gap-2 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition text-sm">
                    PDF
                </button>
                <button onClick={() => exportarEstoqueExcel(materiais)}
                    className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition text-sm">
                    Excel
                </button>
                <button
                    onClick={() => { setForm(materialVazio); setEditando(null); setImagemFile(null); setImagemPreview(null); setModal(true) }}
                                ...
                                <td className="px-4 py-3">
                                    <div className="flex gap-2 justify-center">
                                        ...
                                        {pode.apagarRegistros && (
                                            <button onClick={() => excluir(m.id)} className="text-red-400 hover:text-red-300">
                                                <Trash2 size={16} />
                                            </button>
                                        )}
                                    </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-800 rounded-xl shadow-xl p-6 w-full max-w-md border border-gray-700">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg font-semibold text-gray-100">{editando ? 'Editar Material' : 'Novo Material'}</h3>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-sm text-gray-300">Estoque Mínimo</label>
                                    <input type="number" value={form.minimo} onChange={e => setForm({ ...form, minimo: e.target.value })}
                                        className="w-full bg-gray-700 border border-gray-600 text-gray-100 rounded-lg px-3 py-2 mt-1 focus:outline-none focus:border-blue-500" />
                                </div>
                                <div>
                                    <label className="text-sm text-gray-300">Estoque Máximo</label>
                            ...
                            {/* Upload de imagem */}
                            <div>
                                <label className="text-sm text-gray-300">Foto do Material</label>
                                <div className="mt-1 flex items-center gap-3">
                                    {imagemPreview ? (
                                        <div className="relative">
                                            <img src={imagemPreview} alt="preview" className="w-16 h-16 rounded-lg object-cover" />
                                            <button
                                                onClick={() => { setImagemPreview(null); setImagemFile(null); setForm({ ...form, imagem_url: null }) }}
            ...
            {modalImagem && (
                ...
                >
                    <div className="relative max-w-2xl w-full" onClick={e => e.stopPropagation()}>
                        ...
                        />
                        <p className="text-center text-gray-300 mt-3 font-medium">{modalImagem.descricao}</p>
                    </div>
                </div>
            )}
        </div>
